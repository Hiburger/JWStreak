...
Thanks for considering it! JWS is a small, mostly-solo FLOSS project, so this doc is short on purpose — just enough to keep things consistent.
...
That's it for day-to-day work. A signed **release** build (Android or iOS) needs its own signing setup — see the "Build de production" section of the [README](README.md) for Android, and Xcode's Signing & Capabilities for iOS (a free Apple ID works for local device testing; a paid Apple Developer account is only needed for TestFlight/App Store distribution).
...
If you touched anything under `lib/screens/`, `lib/widgets/`, or notification/deep-link code, please actually run the app (Android and, if you can, iOS) rather than relying on `flutter test` alone — a lot of what JWS does (reminders, deep links, the app lock, tablet layouts) only really shows itself on a real build.

## Code style

- Comments should explain **why**, not what — the code itself should make the *what* obvious. If you find yourself writing "this creates a list of chapters," delete it; if you're writing "this has to run *before* the DB read because X," keep it.
- No comments referencing a specific bug, issue number, or PR — those rot as the code moves on. Explain the constraint itself instead.
...
If you spot a wrong answer, a typo, or an ambiguous question (I wrote these myself and I'm human), either open an issue or a PR — whichever's easier for you.
...
Missing either one means the app will silently mix languages (UI in your new language, quizzes still in French, or vice versa) — please don't ship half of one.

## Reporting bugs / privacy

This app is offline-first and stores everything locally — if you're reporting a bug, please don't paste your personal notes or reading history into a public issue. A description of what you tapped and what happened is almost always enough; screenshots of the UI are fine, screenshots of your notes generally aren't necessary.

For anything sensitive, email **contact@olynthe.org** instead of opening a public issue.

## License

JW Streak is GPLv3 (see `LICENSE`). By contributing, you agree your contribution is licensed under the same terms.

---

# Contribuer à JW Streak

Merci d'y penser ! JWS est un petit projet FLOSS, plutôt solo, donc ce document reste volontairement court — juste de quoi garder une cohérence.
...
- Pas de commentaires faisant référence à un bug, un numéro d'issue ou de PR précis — ça devient obsolète avec le temps. Expliquez plutôt la contrainte elle-même.
...
Si vous repérez une mauvaise réponse, une coquille, ou une question ambiguë (je les ai écrites moi-même et je suis humain), ouvrez une issue ou une PR, selon ce qui vous arrange.
...
L'absence de l'un des deux fait que l'app mélangera silencieusement les langues (interface dans la nouvelle langue, quiz toujours en français, ou l'inverse) — merci de ne pas livrer la moitié d'une langue.

## Signaler un bug / vie privée

Cette app est locale et hors-ligne par nature, tout est stocké sur l'appareil — si vous signalez un bug, merci de ne pas coller vos notes personnelles ou votre historique de lecture dans une issue publique. Une description de ce que vous avez fait et de ce qui s'est passé suffit presque toujours ; des captures d'écran de l'interface sont bienvenues, celles de vos notes ne sont généralement pas nécessaires.

Pour tout ce qui est sensible, écrivez à **contact@olynthe.org** plutôt que d'ouvrir une issue publique.

## Licence

JW Streak est sous licence GPLv3 (voir `LICENSE`). En contribuant, vous acceptez que votre contribution soit sous la même licence.
