...
void main() {
  testWidgets('multiple choice: tapping the right option scores and shows Next', (
    WidgetTester tester,
  ) async {
    int? score;
    int? total;
  ...
  testWidgets('free text: wrong answer reveals the correct one, right answer scores', (
    WidgetTester tester,
  ) async {
    int? score;
    await _pump(
  ...
  testWidgets('word bank: tapping chips in order assembles and scores correctly', (
    WidgetTester tester,
  ) async {
    int? score;
    await _pump(
  ...
  testWidgets('scores multiple questions across a play-through', (
    ...
    final QuizQuestion firstOfTwo =
        find.text('Q1').evaluate().isNotEmpty
            ? const QuizQuestion(
                text: 'Q1',
                options: <String>['Right', 'Wrong'],
                correctIndex: 0,
